...
public class RestApiSpotClient : RestApiClient
{
    // Api
    ...
    internal RestApiSpotClient(GateRestApiClient root) : base(root.ClientOptions)
    {
        RootClient = root;
    ...
    public async Task<RestCallResult<SpotOrder>> PlaceOrderAsync(SpotOrderRequest request, CancellationToken ct = default)
    {
        ...
        parameters.AddOptionalParameter("price", request.Price?.ToString(CI));
